...
export const tasksApi = {
  ...
  async getTasks(
    ...
    return apiClient.get<TaskListResponse>(
      API_ENDPOINTS.HOUSEHOLDS.TASKS(householdId),
      { params: filters }
    );
  },
  ...
  async unassignTask(
    ...
    return apiClient.delete<{ success: boolean }>(
      `${API_ENDPOINTS.TASKS.ASSIGN(taskId)}/${userId}`
    );
  },
  ...
  async markCompleteWithPhoto(
    ...
    return apiClient.post<TaskCompletionResponse>(
      `${API_ENDPOINTS.TASKS.COMPLETE(taskId)}/with-photo`,
      formData,
      {
        headers: {
  ...
  async requestSwap(
    taskId: UUID, 
    data: CreateTaskSwapRequest
  ): Promise<ApiResponse<TaskSwapRequest>> {
    return apiClient.post<TaskSwapRequest>(
      API_ENDPOINTS.TASKS.SWAP_REQUEST(taskId),
      data
    );
  },
  ...
  async getSwaps(householdId: UUID): Promise<ApiResponse<TaskSwapWithDetails[]>> {
    return apiClient.get<TaskSwapWithDetails[]>(
      API_ENDPOINTS.HOUSEHOLDS.TASK_SWAPS(householdId)
    );
  },

  async cancelSwap(swapId: UUID): Promise<ApiResponse<{ success: boolean }>> {
    return apiClient.delete<{ success: boolean }>(
      `/api/task-swaps/${swapId}`
    );
  },
  ...
  async bulkAssign(
    taskIds: UUID[], 
    userId: UUID
  ): Promise<ApiResponse<{ success: boolean; assigned_count: number }>> {
    return apiClient.post<{ success: boolean; assigned_count: number }>(
      '/api/tasks/bulk-assign',
      { task_ids: taskIds, user_id: userId }
    );
  },

  async bulkComplete(
    taskIds: UUID[]
  ): Promise<ApiResponse<{ success: boolean; completed_count: number }>> {
    return apiClient.post<{ success: boolean; completed_count: number }>(
  ...
  async bulkDelete(
    ...
    return apiClient.post<{ success: boolean; deleted_count: number }>(
  ...
  async createFromTemplate(
    ...
    return apiClient.post<Task>(
      `/api/tasks/templates/${templateId}/create`,
      { household_id: householdId, ...overrides }
    );
  },
  ...
  async getTaskStats(
    ...
    return apiClient.get(
      `${API_ENDPOINTS.HOUSEHOLDS.TASKS(householdId)}/statistics`,
      { params: { period } }
    );
  },
  ...
  async createRecurringTask(
    ...
    return apiClient.post<Task>(
      `${API_ENDPOINTS.HOUSEHOLDS.TASKS(householdId)}/recurring`,
      data
    );
  },
  ...
  async pauseRecurringTask(taskId: UUID): Promise<ApiResponse<{ success: boolean }>> {
    return apiClient.post<{ success: boolean }>(
      `${API_ENDPOINTS.TASKS.GET(taskId)}/pause-recurrence`
    );
  },

  async resumeRecurringTask(taskId: UUID): Promise<ApiResponse<{ success: boolean }>> {
    return apiClient.post<{ success: boolean }>(
      `${API_ENDPOINTS.TASKS.GET(taskId)}/resume-recurrence`
    );
  },
  ...
  async addComment(
    ...
    return apiClient.post(
      `${API_ENDPOINTS.TASKS.GET(taskId)}/comments`,
      { comment }
    );
  },

  async getComments(taskId: UUID): Promise<ApiResponse<Array<{
    id: UUID;
    comment: string;
    user_name: string;
    created_at: string;
