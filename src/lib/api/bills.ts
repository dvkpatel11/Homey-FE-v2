...
export const billsApi = {
  ...
  async getBills(
    ...
    return apiClient.get<BillListResponse>(
      API_ENDPOINTS.HOUSEHOLDS.BILLS(householdId),
      { params: filters }
    );
  },
  ...
  async updateBillSplit(
    ...
    return apiClient.put<BillSplit[]>(
      API_ENDPOINTS.BILLS.SPLIT(billId),
      { splits }
    );
  },
  ...
  async recordPayment(data: RecordPaymentRequest): Promise<ApiResponse<PaymentResponse>> {
    return apiClient.post<PaymentResponse>(
      API_ENDPOINTS.BILL_SPLITS.PAYMENT(data.split_id),
      data
    );
  },
  ...
  async uploadReceipt(
    ...
    return apiClient.post<{ receipt_url: string }>(
      `${API_ENDPOINTS.BILLS.GET(billId)}/receipt`,
      formData,
      {
        headers: {
  ...
  async settleBalance(
    ...
    return apiClient.post<{ success: boolean }>(
      `${API_ENDPOINTS.HOUSEHOLDS.BALANCES(householdId)}/settle`,
      {
  ...
  async bulkMarkPaid(
    ...
    return apiClient.post<{ success: boolean; marked_count: number }>(
      '/api/bills/bulk-mark-paid',
      { bill_ids: billIds }
    );
  },
  ...
  async createRecurringBill(
    ...
    return apiClient.post<Bill>(
      `${API_ENDPOINTS.HOUSEHOLDS.BILLS(householdId)}/recurring`,
      data
    );
  },
  ...
  async pauseRecurringBill(billId: UUID): Promise<ApiResponse<{ success: boolean }>> {
    return apiClient.post<{ success: boolean }>(
      `${API_ENDPOINTS.BILLS.GET(billId)}/pause-recurrence`
    );
  },

  async resumeRecurringBill(billId: UUID): Promise<ApiResponse<{ success: boolean }>> {
    return apiClient.post<{ success: boolean }>(
      `${API_ENDPOINTS.BILLS.GET(billId)}/resume-recurrence`
    );
  },
  ...
  async getBillTemplates(): Promise<ApiResponse<Array<{
    id: UUID;
    name: string;
    description: string;
    category: string;
  ...
  async generateExpenseReport(
    ...
    return apiClient.post(
      `${API_ENDPOINTS.HOUSEHOLDS.BILLS(householdId)}/report`,
      options
    );
  },
  ...
  async getBillStats(
    ...
    return apiClient.get(
      `${API_ENDPOINTS.HOUSEHOLDS.BILLS(householdId)}/statistics`,
      { params: { period } }
    );
  },
