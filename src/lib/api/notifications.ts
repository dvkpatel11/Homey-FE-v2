...
export const notificationApi = {
  ...
  async registerDevice(deviceToken: DeviceToken): Promise<ApiResponse<{ success: boolean }>> {
    return apiClient.post<{ success: boolean }>(
      API_ENDPOINTS.NOTIFICATIONS.DEVICE,
      deviceToken
    );
  },
  ...
  async unregisterDevice(token: string): Promise<ApiResponse<{ success: boolean }>> {
    return apiClient.delete<{ success: boolean }>(
      API_ENDPOINTS.NOTIFICATIONS.REMOVE_DEVICE(token)
    );
  },

  async getRegisteredDevices(): Promise<ApiResponse<Array<{
    id: UUID;
    platform: string;
    created_at: string;
    last_used: string;
  ...
  async sendTemplateNotification(
    ...
    return apiClient.post<{ success: boolean; sent_count: number }>(
      `/api/notifications/templates/${templateId}/send`,
      {
        user_ids: userIds,
        variables,
      }
