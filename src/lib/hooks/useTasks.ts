...
export const useTasks = () => {
  ...
  const respondToSwap = async (swapId: UUID, accept: boolean): Promise<boolean> => {
    try {
      const response = accept 
        ? await tasksApi.acceptSwap(swapId)
        : await tasksApi.declineSwap(swapId);
  ...
  const myPendingSwaps = state.swaps.filter(swap => 
    swap.to_user.id === user?.id && swap.status === 'pending'
  );

  const myOutgoingSwaps = state.swaps.filter(swap => 
    swap.from_user.id === user?.id
  );
